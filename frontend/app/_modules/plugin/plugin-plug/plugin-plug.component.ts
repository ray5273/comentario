...
export class PluginPlugComponent implements AfterContentInit, OnChanges {
        ...
        this.ping$
            ...
            .pipe(
                switchMap(() => {
                    ...
                    const id = this.plug?.pluginId ?? (this.route.snapshot.data as PluginRouteData)?.plugin.id;
                    this.plugTag = this.plug?.componentTag ?? (this.route.snapshot.data as PluginRouteData)?.plug.componentTag;
                    if (!id || !this.plugTag) {
                        return EMPTY;
                    }
    ...
    private removeElement(): void {
        if (this.plugElement && this.elementHost) {
            (this.elementHost.nativeElement as HTMLElement).removeChild(this.plugElement);
